...
private fun monthWidgetHeatColorRes(ratio: Float, hasSpending: Boolean): Int {
...
private fun MonthWidgetDayCell(cell: MonthWidgetCalendarCellState) {
	...
	Box(
		modifier = GlanceModifier
			...
			.cornerRadius(3.dp)
			.background(
				if (hasDay) monthWidgetHeatColorRes(
					cell.ratio,
					cell.hasSpending
		...
		updateAppWidgetState(context, glanceId) { prefs ->
			...
			for (day in 1..MONTH_WIDGET_MAX_DAYS) {
				...
				val ratio = if (spending != null) {
					val amountRatio = if (spending.budget > BigDecimal.ZERO) {

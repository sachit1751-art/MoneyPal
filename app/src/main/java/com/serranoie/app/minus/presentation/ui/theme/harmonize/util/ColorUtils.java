...
package com.serranoie.app.minus.presentation.ui.theme.harmonize.util;

/**
 * Color science utilities used by the HCT / CAM16 pipeline.
 *
 * <p>Trimmed to the surface the rest of `harmonize/` actually calls:
 * sRGB ↔ linear-RGB ↔ XYZ conversion and L* ↔ Y conversion. The Lab
 * conversion and per-channel ARGB accessors are removed — nothing in
 * the project touches Lab and `redFromArgb`/`greenFromArgb`/`blueFromArgb`
 * are still kept as private helpers for the XYZ round-trip.
 */
public class ColorUtils {
  ...
  public static double[] xyzFromArgb(int argb) {
    double r = linearized(redChannel(argb));
    double g = linearized(greenChannel(argb));
    double b = linearized(blueChannel(argb));
  ...
  private static int greenChannel(int argb) {
    return (argb >> 8) & 255;
  }

  private static int blueChannel(int argb) {
    return argb & 255;
  }
}

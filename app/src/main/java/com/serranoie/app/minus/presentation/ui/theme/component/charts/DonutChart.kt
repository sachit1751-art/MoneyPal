...
fun DonutChart(
	...
	// Handle empty items list to avoid UnsupportedOperationException
	if (items.isEmpty()) {
		return
	}
	...
	Canvas(modifier = modifier) {
		...
		var itemAngles = items.map {
			it.amount.divide(total, 5, RoundingMode.HALF_DOWN).multiply(360.toBigDecimal())
				.toFloat()
		}

		val shareAngle = itemAngles.filter { it < minSweepAngle }.map { minSweepAngle - it }
		...
		items.forEachIndexed { index, tag ->
			...
			drawArc(
				...
				style = Stroke(
					width = strokeWidth, cap = StrokeCap.Butt
				),
			)
